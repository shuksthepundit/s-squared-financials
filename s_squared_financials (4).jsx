...
export default function SSquaredFinancials() {
  ...
  const cfdCatalog = [
    ...majorPairRows.map((p) => ({ sym: p.pair, name: `${p.pair} Spot FX`, category: "FX", change: p.netPct / 8, demand: p.demand, supply: p.supply, netPct: p.netPct })),
    ...minorPairRows.map((p) => ({ sym: p.pair, name: `${p.pair} Spot FX`, category: "FX", change: p.netPct / 8, demand: p.demand, supply: p.supply, netPct: p.netPct })),
  ...
  return (
    <div style={{ minHeight: "100vh", background: COLORS.bg, color: COLORS.text, fontFamily: F_BODY }}>
      ...
      <div style={{ maxWidth: 1360, margin: "0 auto", padding: "20px 22px 60px" }}>
        ...
        {tab === "cross" && (
          <>
            <SectionTitle icon={Activity} title="Cross-Asset Updates" sub="Crypto · Commodities · Agricultural · Indices · Futures — price, change, and daily demand vs. supply ($B)" />
        ...
        {tab === "cfd" && (
          <>
            ...
            <Panel style={{ overflow: "hidden" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1.1fr 0.75fr 1.25fr 0.75fr 0.85fr 1.05fr 0.85fr", padding: "10px 16px", fontSize: 11, color: COLORS.mutedDim, fontWeight: 600, borderBottom: `1px solid ${COLORS.border}` }}>
                <span>INSTRUMENT</span><span>CATEGORY</span><span>DEMAND / SUPPLY</span><span>CHANGE</span><span>PRESSURE</span><span>LEVERAGE (INDICATIVE)</span><span>SESSION</span>
              </div>
              {filteredCfd.map((c, i) => (
                <div key={c.sym + i} style={{ display: "grid", gridTemplateColumns: "1.1fr 0.75fr 1.25fr 0.75fr 0.85fr 1.05fr 0.85fr", padding: "11px 16px", alignItems: "center", borderBottom: `1px solid ${COLORS.border}`, fontSize: 12.5 }}>
                  <span>
